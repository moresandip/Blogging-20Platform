...
export const getAdminStats: RequestHandler = async (req, res) => {
  try {
    ...
    const totalComments = mockBlogs.reduce((sum: number, blog: any) => sum + (blog.comments || 0), 0);
    const totalLikes = mockBlogs.reduce((sum: number, blog: any) => sum + (blog.likes || 0), 0);
